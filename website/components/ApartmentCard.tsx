...
export function ApartmentCard({ apartment }: ApartmentCardProps) {
  ...
  return (
    <div
      className="bg-card overflow-hidden rounded-xl border border-neutral-200 shadow-sm transition-transform hover:-translate-y-0.5 hover:shadow-2xl"
    ...
    >
      ...
      <div className="p-4">
            ...
            {Array.from({ length: 5 }).map((_, index) => {
              ...
              return (
                <svg
                  key={index}
                  viewBox="0 0 20 20"
                  className={`h-4 w-4 fill-current ${isFilled ? 'text-amber-500' : 'text-muted-foreground'}`}
                  aria-hidden="true"
                >
        ...
        <h4 className="text-card-foreground mb-2 font-semibold">{apartment.name}</h4>
        {apartment.capacity && (
          <p className="text-muted-foreground mb-2 text-sm">
            Guests:{' '}
            {typeof apartment.capacity.minGuests === 'number' && apartment.capacity.minGuests}
            {typeof apartment.capacity.maxGuests === 'number' &&
              apartment.capacity.maxGuests > (apartment.capacity.minGuests ?? 0) &&
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          {apartment.slug && (
            <a
              ...
              className="bg-primary focus-visible:ring-ring inline-flex items-center rounded-md px-3 py-1.5 text-sm font-medium !text-white shadow visited:!text-white hover:!text-white hover:opacity-90 focus:!text-white focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none active:!text-white"
